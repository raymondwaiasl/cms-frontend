...
const HadRecordCreation = () => {
  const client = useApi();

  const { closeCurrentOverlay, data: overlayData } = useOverlay('Had Record Creation');
  const { data, updateWidget, config } = useWidget<{ id: string; tableId: string;propertyId: string }>(
    'Had Record Creation'
  );
  ...
  const {data:propertyColumn } = useQuery(
    ['property column', propertyId],
    async () => {
...
export default HadRecordCreation;
